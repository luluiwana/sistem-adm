 ...
 <script src="<?= base_url('files/js/') ?>webcam-easy.js"></script>
 ...
 <script>
     ...
     $(document).on('click', 'td.detailed', function() {
         var tr = $(this).closest('tr');
         var row = table_sm.row(tr);
         ...
         if (row.child.isShown()) {
             ...
             // Open this row
             row.child(format(tr.data('child-ringkasan'), tr.data('child-kategori'), tr.data('child-nama'), tr.data('child-nomor'), tr.data('child-alamat'))).show();
             tr.addClass('shown');
         }
     });

     function format(ringkasan, kategori, nama, nomor, alamat) {
         ...
         return '<table cellpadding="5" cellspacing="0" border="0" style="padding-left:50px;">' +
             ...
             '<tr>' +
             '<td>Isi Ringkasan:</td>' +
             '<td class="text-wrap">' + ringkasan + '</td>' +
             '</tr>' +
             '</table>';
     }
     ...
     var bk = $('#buku_agenda').DataTable();

     function format2(ringkasan, kategori, nama, nomor, kode, primer, sekunder, tersier, folder, berkas) {
         // `d` is the original data object for the row
         return '<table cellpadding="5" cellspacing="0" border="0" style="padding-left:50px;">' +
             '<tr>' +
             '<td>Editor:</td>' +
     ...
     $(document).on('click', 'td.detailed', function() {
         ...
         if (row.child.isShown()) {
             ...
             // Open this row
             row.child(format2(tr.data('child-ringkasan'), tr.data('child-kategori'), tr.data('child-nama'), tr.data('child-nomor'), tr.data('child-kode'), tr.data('child-primer'), tr.data('child-sekunder'), tr.data('child-tersier'), tr.data('child-folder'), tr.data('child-berkas'))).show();
             tr.addClass('shown');
         }
     });
 </script>
